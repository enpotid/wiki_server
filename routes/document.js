...
app.get(`/:docname`, (req, res) => {
    ...
    sql.query(`SELECT * FROM doc WHERE title=$1`, [docname], (err, resdb) => {
        if (err) { throw err; }
        if (resdb.rows.length === 0) { return res.status(404).send({ error: "Document not found" }); }
...
app.post(`/:docname`, async (req, res) => {
    ...
    try {
        // 동일한 title이 존재하는지 확인
        sql.query(checkQuery, [title], (err, docinfo) => { if (err) {
            res.send("Failed to get Document"); }
            if (docinfo.rows.length != 0) {
                sql.query(updateQuery, [title, body], (err, _res) => {
                    if (err) {res.send("Failed To Update Doc")}
                    res.send(`문서 '${title}'가 성공적으로 업데이트되었습니다.`);
                });
            } else {
                // 동일한 title이 없다면 문서 추가
                sql.query(insertQuery, [title, body]);
                res.send(`문서 '${title}'가 성공적으로 추가되었습니다.`);
            }
        });
        // 동일한 title이 존재하면 그 문서 업데이트
    } catch (err) { console.error('문서 추가 실패:', err.stack); }
})
