    ...
    let tables = [
        {
            ...
            colums: [
                ...
                { name: "createdtime", type:"timestamp with time zone", default:"CURRENT_TIMESTAMP"},
                { name: "lastmodifiedtime", type:"timestamp with time zone", default:"CURRENT_TIMESTAMP"},
...
async function Chk_logic(tables) {
    ...
    tables.map(async (tableinfo) => {
        //테이블이 있으면
        if (await IsTableExist(tableinfo.name)) {/*포멧이 올바른지 확인*/await Chk_format(tableinfo); }
        //없으면
        else {console.log(`${tableinfo.name}테이블이 존제하지 않습니다. 테이블을 만들겠습니다.`);
...
async function IsTableExist(tablename) {
    ...
    const tables = await sql.query("SELECT * FROM information_schema.tables WHERE table_name = $1", [tablename])
    if (tables.rows.length === 0) {return false; }
    else {return true;}
}
async function IsColumnExist(tablename, columnname) {
    const tables = await sql.query("SELECT * FROM information_schema.columns WHERE table_name = $1 AND column_name = $2", [tablename, columnname])
    if (tables.rows.length === 0) {
        console.log(`${columnname}이라는 이름이 붙은 칼럼이 ${tablename}테이블 안에 존제하지 않습니다.`); return false; }
    else {return tables.rows;}
}
async function Chk_format(tableformat) {
    ...
    tableformat.colums.map(async (columnformat) => {
        ...
        if (Column != false) {
            if (!columnformat.type) {console.log("컬럼의 타잎 관련 셋팅이 잘못되었습니다"); process.exit(1)}
            else if (realtype != corrtype) {
                console.log(`${columnformat.name} 컬럼의 주어진 컬럼 데이터 타잎은 ${corrtype}지만 실제 타잎은 ${Column[0].data_type}이랍니다.`)
                process.exit(1);
            }
            if (columnformat.keys) {
                const keys = await sql.query("SELECT conname, a.attname, c.relname FROM pg_constraint con JOIN pg_attribute a ON a.attnum = ANY(con.conkey) JOIN pg_class c ON c.oid = con.conrelid WHERE a.attname = $1 AND c.relname = $2 AND con.contype IN ('p', 'u')", [columnformat.name, tablename])
            ...
            if (columnformat.notnull) {
                if (Column[0].is_nullable === "YES") {
                    console.log(`${columnformat.name}은 NOTNULL이 TRUE인 컬럼이여야 합니다.`)
                    process.exit(1)
                }
            }
            if (columnformat.default) {
                if (Column[0].column_default != columnformat.default) {
                    console.log(`${columnformat.name}의 기본값은 ${columnformat.default}여야 한다. 현제 값:${Column[0].column_default}`)
                    process.exit(1);
                }
            }
        } else {
...
function Mk_tableQuery(table) {
    ...
    let columnSQL = table.colums.map(column => {
